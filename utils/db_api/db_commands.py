...
class Database:
    ...
    async def get_categories(self):
        sql = "SELECT DISTINCT category_name FROM products_product"
        return await self.execute(sql, fetch=True)

    async def get_subcategories(self, category_name):
        sql = f"SELECT DISTINCT subcategory_name FROM products_product WHERE category_name='{category_name}'"
        return await self.execute(sql, fetch=True)

    async def count_products(self, category_name, subcategory_name=None):
        if subcategory_name:
            sql = f"SELECT COUNT(*) FROM products_product WHERE category_name='{category_name}' AND subcategory_name='{subcategory_name}'"
        else:
            sql = f"SELECT COUNT(*) FROM products_product WHERE category_name='{category_name}'"
        return await self.execute(sql, fetchval=True)

    async def get_products(self, category_name, subcategory_name):
        sql = f"SELECT * FROM products_product WHERE category_name='{category_name}' AND subcategory_name='{subcategory_name}'"
        return await self.execute(sql, fetch=True)
